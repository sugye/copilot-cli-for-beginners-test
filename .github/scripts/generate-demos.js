...
function setupCopilotWrapper() {
  ...
  let realCopilot;
  try {
    realCopilot = execSync(whichCmd, { encoding: 'utf8' }).split(/\r?\n/)[0].trim();
  } catch (e) {
    throw new Error('copilot binary not found in PATH; please ensure copilot is installed and on PATH');
  }
  mkdirSync(wrapperDir, { recursive: true });
  const wrapperPath = join(wrapperDir, process.platform === 'win32' ? 'copilot.cmd' : 'copilot');
  const wrapperContent = process.platform === 'win32'
    ? `@echo off\r\n"${realCopilot}" --yolo --allow-all-paths %*`
    : `#!/bin/bash\nexec "${realCopilot}" --yolo --allow-all-paths "$@"\n`;
  writeFileSync(wrapperPath, wrapperContent);
  if (process.platform !== 'win32') chmodSync(wrapperPath, '755');
  const delimiter = require('path').delimiter;
  return `${wrapperDir}${delimiter}${process.env.PATH}`;
}
...
// Ensure we restore what we changed when an unhandled error occurs.
main().catch(e => {
  console.error(e);
  cleanupCopilotWrapper();
  try { restorePersonalAgents(typeof agentsWereHidden !== 'undefined' ? agentsWereHidden : []); } catch (err) { console.warn('Failed to restore personal agents:', err && err.message); }
  try { restoreStreamerMode(typeof streamerState !== 'undefined' ? streamerState : null); } catch (err) { console.warn('Failed to restore streamer mode:', err && err.message); }
  process.exit(1);
});
